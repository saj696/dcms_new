...
class Common extends Root_Controller
{
    ...
    public function get_udc()
    {
        ...
        $udcs = Query_helper::get_info($this->config->item('table_uisc_infos'),array('id value', 'uisc_name text'), array('uisc_type = '.$center_type_udc, 'zilla = '.$zilla_id, 'upazilla = '.$upazila_id, 'union = '.$union_id));
        $ajax['status']=true;
        $ajax['system_content'][]=array("id"=>"#digital_center","html"=>$this->load_view("dropdown",array('drop_down_options'=>$udcs),true));
        $this->jsonReturn($ajax);
    }

    public function get_pdc()
    {
        $zilla_id=$this->input->post('zilla_id');
        $municipal_id=$this->input->post('municipal_id');
        $municipal_word_id=$this->input->post('municipal_word_id');
        $center_type_pdc = $this->config->item('ONLINE_MUNICIPAL_WORD_GROUP_ID');

        $pdcs = Query_helper::get_info($this->config->item('table_uisc_infos'),array('id value', 'uisc_name text'), array('uisc_type = '.$center_type_pdc, 'zilla = '.$zilla_id, 'municipal = '.$municipal_id, 'municipalward = '.$municipal_word_id));
        $ajax['status']=true;
        $ajax['system_content'][]=array("id"=>"#digital_center","html"=>$this->load_view("dropdown",array('drop_down_options'=>$pdcs),true));
        $this->jsonReturn($ajax);
    }

    public function get_cdc()
    {
        $zilla_id=$this->input->post('zilla_id');
        $city_corporation_id=$this->input->post('city_corporation_id');
        $city_corporation_word_id=$this->input->post('city_corporation_word_id');
        $center_type_cdc = $this->config->item('ONLINE_CITY_CORPORATION_WORD_GROUP_ID');

        $cdcs = Query_helper::get_info($this->config->item('table_uisc_infos'),array('id value', 'uisc_name text'), array('uisc_type = '.$center_type_cdc, 'zilla = '.$zilla_id, 'citycorporation = '.$city_corporation_id, 'citycorporationward = '.$city_corporation_word_id));
        $ajax['status']=true;
        $ajax['system_content'][]=array("id"=>"#digital_center","html"=>$this->load_view("dropdown",array('drop_down_options'=>$cdcs),true));
        $this->jsonReturn($ajax);
    }
