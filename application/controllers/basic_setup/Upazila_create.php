...
class Upazila_create extends Root_Controller
{
    ...
    private function system_edit($id)
    {
        if($this->permissions['edit'])
        {
            ...
            $data['UpazilaInfo']=Query_helper::get_info($this->config->item('table_upazilas'),'*',array('id ='.$id),1);

            $data['zillas']=Query_helper::get_info($this->config->item('table_zillas'),array('zillaid value', 'zillaname text'), array('zillaid='.$data['UpazilaInfo']['zillaid']));
