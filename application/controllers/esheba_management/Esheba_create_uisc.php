...
class Esheba_create_uisc extends Root_Controller
{
    public $permissions;
    public $message;
    ...
    function __construct()
    {
        ...
        parent::__construct();
        $this->message='';
        $this->permissions=Menu_helper::get_permission('esheba_management/Esheba_create_uisc');
        $this->controller_url='esheba_management/Esheba_create_uisc';
        $this->load->model("esheba_management/Esheba_create_uisc_model");
    ...
    private function system_list()
    {
        if($this->permissions['list'])
        {
            ...
            $ajax['system_content'][]=array("id"=>"#system_wrapper","html"=>$this->load_view("esheba_management/esheba_create_uisc/list","",true));

            if($this->message)
            {
                $ajax['system_message']=$this->message;
            }
            $ajax['system_page_url']=$this->get_encoded_url('esheba_management/Esheba_create_uisc');
    ...
    private function system_add()
    {
        ...
        if($this->permissions['add'])
        {
            ...
            (
                ...
                'website'=>'',
                'service_amount'=>'',
                'status'=>$this->config->item('STATUS_PROPOSED'),
            );

            $ajax['system_content'][]=array("id"=>"#system_wrapper_top_menu","html"=>$this->load_view("top_menu","",true));
            $ajax['system_content'][]=array("id"=>"#system_wrapper","html"=>$this->load_view("esheba_management/esheba_create_uisc/add_edit",$data,true));
            ...
            $ajax['system_page_url']=$this->get_encoded_url('esheba_management/esheba_create_uisc/index/add');
            $this->jsonReturn($ajax);
        }
        else
        {
    ...
    private function system_edit($id)
    {
        ...
        if($this->permissions['edit'])
        {
            ...
            $ajax['system_page_url']=$this->get_encoded_url('esheba_management/esheba_create_uisc/index/edit/'.$id);
            $this->jsonReturn($ajax);
        }
        else
        {
    ...
    private function system_save()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                $service_detail['status']=$this->config->item('STATUS_INACTIVE');

                $this->db->trans_start();  //DB Transaction Handle START

                $service_last_id=Query_helper::add($this->config->item('table_services'),$service_detail);
    ...
    private function check_validation()
    {
        ...
        */
        if($this->Esheba_create_uisc_model->check_existence($this->input->post("service_detail[service_name]"),$this->input->post('id'), "service_name"))
        {
    ...
    public function get_list()
    {
        ...
        if($this->permissions['list'])
        {
            $records = $this->Esheba_create_uisc_model->get_record_list();
        }
