...
class Registered_union_report extends CI_Controller
{
    public $permissions;
    ...
    private function report_list($format="")
    {
        if($format!="pdf")
        {
            ...
            $data['getUiscInfos']=$this->uisc_registration_model->get_uisc_registration_info($division, $zilla, $upazila, $union, $uisc_status);
            $this->load->view('default/report/registered_union_report',$data);
        }
        else
        {
            $data['title']=$this->lang->line("REPORT_UISC_REGISTRATION_TITLE");
            ...
            $uisc_status=$this->input->get('status');
            $data['getUiscInfos']=$this->uisc_registration_model->get_uisc_registration_info($division, $zilla, $upazila, $union, $uisc_status);
            $html = $this->load->view('default/report/registered_union_report',$data, true);
            //echo $html;
