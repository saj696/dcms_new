...
class Service_template extends Root_Controller
{
    ...
    private function dcms_save()
    {
        ...
        else
        {
            ...
            for($i=0; $i<$count; $i++)
            {
                ...
                elseif($receiver_sex == $this->lang->line('DISABILITY_VAL'))
                {
                    $total_tribe++;
                }
                elseif($receiver_sex == $this->lang->line('TRIBE_VAL'))
                {
                    $total_disability++;
                }
            }
    ...
    private function check_validation()
    {
        ...
        if(!strtotime($this->input->post('report_date')))
        {
            $this->message="Report Date Required";
            return false;
        }
        //$this->load->library('form_validation');
        //$this->form_validation->set_rules('report_date','Invoice Date','required|date_valid');
