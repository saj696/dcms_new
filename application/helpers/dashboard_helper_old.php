...
class Dashboard_helper
{
    ...
    public static function get_total_notice_upazila()
    {
        //        $CI = & get_instance();
        //        $CI->db->from($CI->config->item('table_notice_view').' notice_view');
        //        $CI->db->where('notice_view.viewer_user_group',$CI->config->item('UPAZILLA_GROUP_ID'));
        //        $CI->db->where('notice_view.status',$CI->config->item('STATUS_ACTIVE'));
        //        $total = $CI->db->count_all_results();
    ...
    public static function get_number_of_uisc_user_dashboard($gender=null)
    {
        ...
        if($user->user_group_id==$CI->config->item('SUPER_ADMIN_GROUP_ID') || $user->user_group_id==$CI->config->item('A_TO_I_GROUP_ID') || $user->user_group_id==$CI->config->item('DONOR_GROUP_ID') || $user->user_group_id==$CI->config->item('MINISTRY_GROUP_ID'))
        {
            //$CI->db->where('','');
        }
        elseif($user->user_group_id==$CI->config->item('DIVISION_GROUP_ID'))
        {
        ...
        elseif($user->user_group_id==$CI->config->item('UNION_GROUP_ID'))
        {
            $CI->db->where('users.division',$user->division);
            $CI->db->where('users.zilla',$user->zilla);
            $CI->db->where('users.upazila',$user->upazila);
        ...
        elseif($user->user_group_id==$CI->config->item('CITY_CORPORATION_WORD_GROUP_ID'))
        {
            $CI->db->where('users.division',$user->division);
            $CI->db->where('users.zilla',$user->zilla);
            $CI->db->where('users.citycorporation',$user->citycorporation);
        ...
        elseif($user->user_group_id==$CI->config->item('MUNICIPAL_WORD_GROUP_ID'))
        {
            $CI->db->where('users.division',$user->division);
            $CI->db->where('users.zilla',$user->zilla);
            $CI->db->where('users.municipal',$user->municipal);
    ...
    public static function get_number_of_notice($status)
    {
        ...
        $CI->db->where('notice_view.viewer_user_group',$user->user_group_id);
        $total = $CI->db->count_all_results();
        //echo $CI->db->last_query();
