...
class User_approval_model extends CI_Model
{
    ...
    public function get_approval_uisc_detail($entrepreneur_type, $division, $zilla, $upazilla, $municipal, $municipalward, $citycorporation, $citycorporationward, $union, $year, $month, $date, $status)
    {
        ...
        if(strtotime($date)>0)
        {
            ...
            $CI->db->where("uisc.create_date BETWEEN '". strtotime(date('Y-m-d',strtotime($date)).' 00:00')."' AND  '". strtotime(date('Y-m-d',strtotime($date)).' 23:59')."'");
        }

        if($month>0)
        {
            $CI->db->where("MONTH(from_unixtime(uisc.create_date,'%Y-%m-%d'))", $month);
        }

        if($year>0)
        {
            $CI->db->where("YEAR(from_unixtime(uisc.create_date,'%Y-%m-%d'))", $year);
        }
        if($status)
        {
            $CI->db->where('uisc.status', $status);
        ...
        $CI->jsonReturn($uiscs);
        //$CI->jsonReturn($CI->db->last_query());
    }
    ...
    public function fetch_uisc_detail_info($id)
    {
        ...
        $CI->db->join($CI->config->item('table_uisc_infos').' uisc_infos','uisc_infos.id = users.uisc_id', 'INNER');
