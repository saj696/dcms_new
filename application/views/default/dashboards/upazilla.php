...
<script>
    $(function () {
        $('#container').highcharts({
            ...
            title: {
                text: '<?php echo $CI->lang->line('REPORT_TITLE_UNION').' ('.System_helper::Get_Eng_to_Bng($week_start_date).' '.$CI->lang->line('TO').' '.System_helper::Get_Eng_to_Bng($yesterday).')';?>'
            },
            xAxis: {
                categories: [<?php
             $index=0;
            ...
            series: [{
                name : ' <?php echo $CI->lang->line('UNION') ?>',
                data: [<?php
             $index=0;
             foreach($union_infos as $union)
             {
                if($index==0)
                {
                    echo ($union['income'] ? $union['income']/100000 : 0);
                }
                else
                {
                    echo ",".($union['income'] ? $union['income']/100000 : 0);
