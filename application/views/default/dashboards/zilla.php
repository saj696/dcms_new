...
<script>
    $(function () {
        $('#container').highcharts({
            ...
            title: {
                text: '<?php echo $CI->lang->line('REPORT_TITLE_UPAZILLA').' ('.System_helper::Get_Eng_to_Bng($week_start_date).' '.$CI->lang->line('TO').' '.System_helper::Get_Eng_to_Bng($yesterday).')';?>'
            },
            xAxis: {
                categories: [<?php
             $index=0;
             foreach($upazila_info as $upazila)
             {
                if($index==0)
                {
                    echo "'".$upazila['name']."'";
            ...
            series: [{
                name : ' <?php echo $CI->lang->line('UPAZILLA') ?>',
                data: [<?php
             $index=0;
             foreach($upazila_info as $upazila)
             {
                if($index==0)
                {
                    echo ($upazila['income'] ? $upazila['income']/100000 : 0);
                }
                else
                {
                    echo ",".($upazila['income'] ? $upazila['income']/100000 : 0);
