...
<div id="system_content" class="system_content_margin">
    ...
    <form id="system_save_form" action="<?php echo $CI->get_encoded_url('esheba_management/Esheba_create/index/save'); ?>" method="post">
        ...
        <div class="row widget">
            ...
            <div style="" class="row show-grid " id="">
                ...
                <div class="col-sm-4 col-xs-8">
                    ...
                    <select name="service_detail[service_type]" class="form-control">
                        <option value=""><?php echo $this->lang->line('SELECT');?></option>
                        <?php
                        foreach($this->config->item('service_type') as $val=>$service_type)
                        {
                            if($ServiceInfo['service_type']==$val)
                            {
                                ?>
                                <option value="<?php echo $val;?>" selected="selected"><?php echo $service_type;?></option>
                            <?php
                            }
                            else
                            {
                                ?>
                                <option value="<?php echo $val;?>"><?php echo $service_type;?></option>
                            <?php
                            }
                        }
                        ?>
            ...
            <div style="" class="row show-grid">
                ...
                <div class="col-sm-4 col-xs-8">
                    <select name="service_detail[status]" class="form-control" id="module_options">
                        <?php
                        $CI->load_view('dropdown',array('drop_down_default_option'=>false,'drop_down_options'=>array(array('text'=>$CI->lang->line('INACTIVE'),'value'=>$this->config->item('STATUS_INACTIVE')),array('text'=>$CI->lang->line('ACTIVE'),'value'=>$this->config->item('STATUS_ACTIVE'))),'drop_down_selected'=>$ServiceInfo['status']));
                        ?>
                    </select>
                </div>
            </div>
