<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
$CI =& get_instance();
$user=User_helper::get_user();
if($user->user_group_level==$CI->config->item('SUPER_ADMIN_GROUP_ID'))
{
    $CI->load_view('dashboards/super_admin');
}
else if($user->user_group_level==$CI->config->item('A_TO_I_GROUP_ID'))
{
    $CI->load_view('dashboards/super_admin');
}
else if($user->user_group_level==$CI->config->item('DONOR_GROUP_ID'))
{
    $CI->load_view('dashboards/super_admin');
}
else if($user->user_group_level==$CI->config->item('MINISTRY_GROUP_ID'))
{
    $CI->load_view('dashboards/super_admin');
}
else if($user->user_group_level==$CI->config->item('DIVISION_GROUP_ID'))
{
    $CI->load_view('dashboards/super_admin');
    //$CI->load_view('dashboards/division');
}
elseif($user->user_group_level==$CI->config->item('DISTRICT_GROUP_ID'))
{
    $CI->load_view('dashboards/super_admin');
    //$CI->load_view('dashboards/zilla');
}
elseif($user->user_group_level==$CI->config->item('UPAZILLA_GROUP_ID'))
{
    $CI->load_view('dashboards/super_admin');
    //$CI->load_view('dashboards/upazilla');
}
elseif($user->user_group_level==$CI->config->item('MUNICIPAL_GROUP_ID'))
{
    $CI->load_view('dashboards/super_admin');
    //$CI->load_view('dashboards/upazilla');
}
elseif($user->user_group_level==$CI->config->item('CITY_CORPORATION_GROUP_ID'))
{
    $CI->load_view('dashboards/super_admin');
    //$CI->load_view('dashboards/upazilla');
}
elseif($user->user_group_level==$CI->config->item('UISC_GROUP_ID'))
{
    $CI->load_view('dashboards/uisc');
}
else
{
    echo '<br><br><br><br><br>';
}
?>
