                        ...
                        foreach($division['zilla'] as $zilla)
                        {
                            foreach($zilla['city_corporation'] as $city_corporation)
                            {
                                ...
                                <tr style="background: #cccccc">
                                    ...
                                    <td><?php echo System_helper::Get_Eng_to_Bng($city_corporation['number_of_city_corporation']);?></td>
                                    <td><?php echo System_helper::Get_Eng_to_Bng($city_corporation['number_of_uisc']);?></td>
                                    <th><?php if(isset($city_corporation['invoice'])){echo $this->lang->line('DATE');}?></th>
                                    <th><?php if(isset($city_corporation['invoice'])){echo $this->lang->line('INCOME');}?></th>
                                </tr>

                                <?php
                                $invoice_date='';
                                $invoice_amount='';
                                if(isset($city_corporation['invoice']))
                                {
                                    for($i=0; $i<sizeof($city_corporation['invoice']); $i++)
                                    {
                                        if(isset($city_corporation['invoice'][$i]['invoice_date']))
                                        {
                                            $invoice_date = strtotime($city_corporation['invoice'][$i]['invoice_date']);
                                            $invoice_amount = $city_corporation['invoice'][$i]['invoice_amount'];
                                        }
                                        else
                                        {
                                            $invoice_date='';
