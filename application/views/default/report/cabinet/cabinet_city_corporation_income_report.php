...
<div class="container">
    <div class="main_container">
        ...
        <div class="col-lg-12">
            ...
            <p style="margin:15px 0px;text-align: center"><?= System_helper::Get_Eng_to_Bng($from_date) . ' ' . $this->lang->line('FROM_DATE') ?>
                &nbsp;&nbsp;<?= System_helper::Get_Eng_to_Bng($to_date) . ' ' . $this->lang->line('TO_DATE') ?></p>
            <table class="table table-responsive table-bordered">
                <thead>
                <!--                <tr>-->
                <!--                    <th colspan="21" class="text-center">-->
                <!--                        --><?php //echo System_helper::Get_Eng_to_Bng($from_date);?><!-- -->
                <?php //echo $this->lang->line('TO');?><!-- --><?php //echo System_helper::Get_Eng_to_Bng($to_date);?>
                <!--                    </th>-->
                <!--                </tr>-->
                <tr>
                    ...
                    <th><?php echo $this->lang->line('TOTAL_CDC_UPLOAD_REPORT_PERCENTAGE'); ?></th>
                ...
                if (empty($report)) {
                    ...
                    foreach ($report as $division) {
                        foreach ($division['zilla'] as $zilla) {
                            ...
                            if (!empty($total_cdc) && !empty($total_report_upload)) {
                                $total_report_upload_percentage = ($total_report_upload / $cdc[$zilla['zilla_id']]);
                            } else {
                                $total_report_upload_percentage = 0;
                            }

                            if (!empty($total_cdc) && !empty($total_income)) {
                                $total_income_percentage = ($total_income / $cdc[$zilla['zilla_id']]);
                            } else {
                                $total_income_percentage = 0;
                            }

                            if (!empty($total_cdc) && !empty($total_service_holder)) {
                                $total_service_holder_percentage = ($total_service_holder / $cdc[$zilla['zilla_id']]);
                            } else {
                                $total_service_holder_percentage = 0;
                            }
                    ...
                    <tr style="background: #EEEEEE">
                        ...
                        <th><?php echo System_helper::Get_Eng_to_Bng(number_format(array_sum($cdc))); ?></th>
